...
@interface AwemeListCell()<SendTextDelegate, HoverTextViewDelegate>
...
- (void)initSubViews {
    ...
    _musicName.contentMode = UIViewContentModeCenter;
    ...
    [_musicName mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.bottom.equalTo(self.musicIcon);
        make.right.equalTo(_musicAlum.mas_left).offset(-10);
    }];
...
// update method
- (void)initData:(YBVideoModel *)video {
    _videoModel = video;
    
    [_nickName setText:[NSString stringWithFormat:@"@%@", video.videoAuthor]];
    [_desc setText:video.videoTitle];
    [_musicName setText:[NSString stringWithFormat:@"%@ - %@", video.videoTitle, video.videoAuthor]];
    [_favoriteNum setText: video.videoFavorites];
    [_commentNum setText:video.videoComments];
    [_shareNum setText:[NSString formatCount: 0]];
